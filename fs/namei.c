...
int inode_permission(struct inode *inode, int mask)
{
	...
	retval = security_inode_permission(inode,
					   mask & (MAY_READ|MAY_WRITE|MAY_EXEC|MAY_APPEND));

	provenance_inode_permission(inode,
				    mask & (MAY_READ|MAY_WRITE|MAY_EXEC|MAY_APPEND));
...
static int exec_permission_lite(struct inode *inode)
{
	...
	return ret;
ok:
	ret = security_inode_permission(inode, MAY_EXEC);
	provenance_inode_permission(inode, MAY_EXEC);
	return ret;
}
...
int vfs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	if (d_mountpoint(dentry))
		...
		error_prov = provenance_inode_unlink(dir, dentry);
		if (!error && !error_prov)
...
int vfs_link(struct dentry *old_dentry, struct inode *dir, struct dentry *new_dentry)
{
	...
	error = provenance_inode_link(old_dentry, dir, new_dentry);
	if (error)
		return error;
			  ...
			  struct inode *new_dir, struct dentry *new_dentry)
{
	...
	error = provenance_inode_rename(old_dir, old_dentry, new_dir, new_dentry);
	if (error)
		return error;
