 ...
 *  Added provenance hook support: Adam M. Bates <amb@cs.uoregon.edu>
...
static struct super_block *alloc_super(struct file_system_type *type)
{
	...
	if (s) {
	        if (security_sb_alloc(s) || provenance_sb_alloc(s)) {
			/*
			 * We cannot call security_sb_free() without security_sb_alloc() and
			 * provenance_sb_alloc() succeeding. So bail out manually
			 */
			kfree(s);
			s = NULL;
			goto out;
...
vfs_kern_mount(struct file_system_type *type, int flags, const char *name, void *data)
{
	...
	mnt->mnt_sb->s_flags |= MS_BORN;
 	...
 	error = security_sb_kern_mount(mnt->mnt_sb, flags, provdata);
 	if (error)
 		goto out_sb;

	/*
	 * Allocate provenance data
	 */
	if (data && !(type->fs_flags & FS_BINARY_MOUNTDATA)) {
	        provdata = alloc_provdata();
		if (!provdata)
		  goto out_sb;

		error = provenance_sb_copy_data(data, provdata);
		if (error)
			goto out_free_provdata;
	}
 	error = provenance_sb_kern_mount(mnt->mnt_sb, flags, provdata);
 	if (error)
 		goto out_free_provdata;
