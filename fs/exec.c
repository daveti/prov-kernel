...
void install_exec_creds(struct linux_binprm *bprm)
{
	...
	provenance_bprm_committing_creds(bprm);
	...
	provenance_bprm_committed_creds(bprm);
	mutex_unlock(&current->cred_guard_mutex);
}
...
int prepare_binprm(struct linux_binprm *bprm)
{
	...
	/* fill in binprm provenance blob */
	retval = provenance_bprm_set_creds(bprm);
	if (retval)
		return retval;
...
int search_binary_handler(struct linux_binprm *bprm,struct pt_regs *regs)
{
	...
	retval = provenance_bprm_check(bprm);
	if (retval)
		return retval;
