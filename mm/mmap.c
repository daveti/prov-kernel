 ...
 * Added provenance module support      Adam M. Bates <amb@cs.uoregon.edu>
			...
			unsigned long flags, unsigned long pgoff)
{
	...
	error = provenance_file_mmap(file, reqprot, prot, flags, addr, 0);
	if (error)
		return error;
				   ...
				   unsigned long address)
{
	...
	error = provenance_file_mmap(NULL, 0, 0, 0, address, 1);
	if (error)
		return error;
...
unsigned long do_brk(unsigned long addr, unsigned long len)
{
	...
	error = provenance_file_mmap(NULL, 0, 0, 0, addr, 1);
	if (error)
		return error;
			    ...
			    unsigned long vm_flags, struct page **pages)
{
	...
	ret = provenance_file_mmap(NULL, 0, 0, 0, vma->vm_start, 1);
	if (ret)
		goto out;
