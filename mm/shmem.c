 ...
 *
 * Added provenance module support:
...
shmem_mknod(struct inode *dir, struct dentry *dentry, int mode, dev_t dev)
{
	...
	if (inode) {
		...
		/* Check provenance init */
		error_prov = provenance_inode_init_provenance(inode, dir, NULL, NULL,
							      NULL);
		if (error_prov) {
			if (error_prov != -EOPNOTSUPP) {
			  iput(inode);
			  return error_prov;
			}
		}
...
static int shmem_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	error = provenance_inode_init_provenance(inode, dir, NULL, NULL,
						 NULL);
	if (error) {
		if (error != -EOPNOTSUPP) {
			iput(inode);
