...
static int unix_bind(struct socket *sock, struct sockaddr *uaddr, int addr_len)
{
	...
	if (sunaddr->sun_path[0]) {
		...
		err = (security_path_mknod(&nd.path, dentry, mode, 0)
		       || provenance_path_mknod(&nd.path, dentry, mode, 0));
		if (err)
			goto out_mknod_dput;
...
restart:
		...
		err = provenance_unix_may_send(sk->sk_socket, other->sk_socket);
		if (err)
			goto out_unlock;
...
restart:
	...
	err = (security_unix_stream_connect(sock, other->sk_socket, newsk)
	       || provenance_unix_stream_connect(sock, other->sk_socket, newsk));
	if (err) {
		unix_state_unlock(sk);
		goto out_unlock;
	}
