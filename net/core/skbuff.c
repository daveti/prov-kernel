			    ...
			    int fclone, int node)
{
	...
	memset(&shinfo->hwtstamps, 0, sizeof(shinfo->hwtstamps));
	  ...
	  goto nosec;
        if (provenance_skb_shinfo_alloc(skb, 0, gfp_mask))
	  goto nosec;
...
static void skb_release_data(struct sk_buff *skb)
{
	if (!skb->cloned ||
	    !atomic_sub_return(skb->nohdr ? (1 << SKB_DATAREF_SHIFT) + 1 : 1,
   	        ...
   	        provenance_skb_shinfo_free(skb, 0);
...
int skb_recycle_check(struct sk_buff *skb, int skb_size)
{
	...
	provenance_skb_shinfo_free(skb, 1);
	skb_release_head_state(skb);
	shinfo = skb_shinfo(skb);
	atomic_set(&shinfo->dataref, 1);
	shinfo->nr_frags = 0;
	...
	ret = !(security_skb_shinfo_alloc(skb, 1, GFP_ATOMIC)
		|| provenance_skb_shinfo_alloc(skb, 1, GFP_ATOMIC));
	return ret;
}
		     ...
		     gfp_t gfp_mask)
{
	...
	if (fastpath) {
		kfree(skb->head);
	        ...
	        if (provenance_skb_shinfo_copy(skb,
					     (struct skb_shared_info *)(data + size),
					     gfp_mask))
		        goto nofrags;
