 ...
 *              Adam Bates      :       Added provenance module support
		    ...
		    char __user *optval, int __user *optlen)
{
	...
	case SO_PEERSEC:
	  return (security_socket_getpeersec_stream(sock, optval, optlen, len)
		  || provenance_socket_getpeersec_stream(sock, optval, optlen, len));
...
#endif
#ifdef CONFIG_PROVENANCE
	nsk->sk_provenance = pptr;
	provenance_sk_clone(osk, nsk);
#endif
}
		...
		int family)
{
	...
	if (sk != NULL) {
		...
		if (provenance_sk_alloc(sk, family, priority))
			goto out_free_sec;
