...
#ifndef _SECURITY_PROVENANCE_INTERPRET_H
#define _SECURITY_PROVENANCE_INTERPRET_H

#include <linux/provenance_proto.h>

#define PATH_LENGTH 255

static inline int provenance_strcpyif(char * buf, int buflen, char * str, int strlen){
  int rv = 0;
  if( strlen < buflen ){
    ...
    printk(KERN_ERR "Bates: provenance_strcpyif, buffer of insufficient length. truncating. (%s)\n",buf);
  return rv;
}
...
int provenance_dentry_mountpoint(struct dentry *d, char *buf, int buflen) {
  ...
  list_for_each(pos,head){
	  ...
	  if((char)name->name[0] != '/')
	    len += provenance_strcpyif(buf + len, buflen - len, "/", 1);

	  len += provenance_strcpyif(buf + len, buflen - len, (char *)name->name, name->len);

	  counter += 1;	  
	}   
  }
  if(counter > 1)	    
    printk(KERN_ERR "Bates: provenance_dentry_mountpoint, TWO MOUNTPOINTS = %s\n",buf);
  else if(counter == 0)
    printk(KERN_ERR "Bates: provenance_dentry_mountpoint, NO MOUNTPOINTS for root=%s and dentry=%s\n",
	   (&root->d_name)->name,
	   (&d->d_name)->name);
 ...
 */
int provenance_dentry_path_rec(struct dentry *dentry, char *buf, int buflen){

  struct qstr * name = &dentry->d_name;
  int len = 0;

  if(!IS_ROOT(dentry)){
    len += provenance_dentry_path_rec(dentry->d_parent, buf + len, buflen - len);
...
int provenance_dentry_path(struct dentry *dentry, char *buf, int buflen)
{
        int len = 0;

	spin_lock(&dcache_lock);
	len += provenance_dentry_path_rec(dentry,buf,buflen);
	spin_unlock(&dcache_lock);

	return len;
}

int provenance_inode_to_dentry(struct inode * inode, char *buf, int buflen)
{
	...
	list_for_each(pos,head){
	  ...
	  len += provenance_dentry_mountpoint(alias, buf + len, buflen - len);
	  
	  //Copy in the  dentry full buf
	  len += provenance_dentry_path(alias, buf + len, buflen - len);
	...
	/*
	  if(counter > 1)
	    printk(KERN_ERR "Bates: provenance_inode_permission, TWO DENTRIES: %s\n",buf);
	  else
	    printk(KERN_ERR "Bates: provenance_inode_permission, filename=%s\n",buf);
	*/
